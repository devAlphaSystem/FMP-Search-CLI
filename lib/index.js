...
export async function search(query, options = {}) {
  const { limit = DEFAULT_LIMIT, timeout = DEFAULT_TIMEOUT, sort, concurrency: rawConcurrency = DEFAULT_CONCURRENCY, city, category, minPrice, maxPrice, radius, strict = false, noRateLimit = false, noDetails = false, onFirstResponse = null, onErrorResponse = null } = options;
  const concurrency = noRateLimit ? rawConcurrency : Math.min(rawConcurrency, RATE_LIMIT_CONCURRENCY);
  let _requestCount = 0;
  log("SEARCH", `search("${query}") called`, { limit, sort, city, category, minPrice, maxPrice, radius, strict, noRateLimit, noDetails, concurrency });
  ...
  if (!pageData || !pageData.listings || pageData.listings.length === 0) {
    if (looksLikeBlockPage(firstHtml)) {
      ...
      throw new Error("Access blocked by Facebook Marketplace — your IP is rate limited. Try again later, or avoid using --no-rate-limit.");
    }
    if (onErrorResponse) {
      try {
  ...
  log("SEARCH", `page 1 parsed: ${pageData.listings.length} listings -> ${items.length} valid items`);
  ...
  while (items.length < limit && hasNextPage && endCursor && pagesFetched < MAX_PAGES) {
    ...
    pagesFetched++;
    log("SEARCH", `page ${pagesFetched} fetched: ${pageResult.listings.length} new listings, items total=${items.length}`);
  }
          ...
          try {
              ...
              if (idx >= 0) {
                ...
                if (textIdx >= 0 && textIdx - idx <= 200) {
                  ...
                  for (let k = start; k < html.length; k++) {
                    if (html[k] === '"') {
                      let backslashes = 0;
                      for (let b = k - 1; b >= start && html[b] === "\\"; b--) backslashes++;
                      if (backslashes % 2 === 0) {
                        end = k;
                        break;
                      }
            ...
            let createdAt = null;
            {
              ...
              if (ctMatch) {
                ...
                if (ts > 1000000000) createdAt = new Date(ts * 1000).toISOString();
              }
            }

            log("DETAIL", `  -> ok (desc=${!!description}, images=${images.length}, seller=${!!sellerName}, condition=${!!condition}, date=${!!createdAt})`);
            return {
              description,
  ...
  log("SEARCH", `search() done: ${items.length} items returned, pages=${pagesFetched}, requests=${_requestCount}, capped=${capped}`);
  return {
    items,
    query: {
      text: query,
    ...
    stats: {
      ...
      pageRequests: pagesFetched,
      detailRequests: _requestCount - pagesFetched,
    },
  };
}
...
export async function searchRaw(query, options = {}) {
  ...
  log("SEARCH", `searchRaw("${query}") called`, { timeout, city, category });
...
/**
 * Lazily builds and returns a map from normalised city names to city config objects,
 * used to support inexact (accent-insensitive) city lookups.
 *
 * @returns {Map<string, {vanity: string, name: string, lat: number, lng: number, uf: string}>}
 */
...
/**
 * Resolves a city slug or name to its configuration object.
 * Supports both exact slug keys and normalised name lookups.
 *
 * @param {string} input - City slug or name (e.g. "saopaulo" or "São Paulo").
 * @returns {{vanity: string, name: string, lat: number, lng: number, uf: string}}
 * @throws {Error} If the city is not in the known list.
 */
...
/**
 * Fetches a URL using Node's native fetch API with browser-like headers.
 *
...
/**
 * Fetches a URL using curl as a fallback transport when native fetch fails.
 *
 * @param {string} url - Target URL.
 * @param {number} timeout - Request timeout in milliseconds.
 * @returns {Promise<string>} Response body.
 * @throws {Error} If curl exits with an HTTP error status.
 */
async function fetchWithCurl(url, timeout) {
  ...
  const retryAfterStr = lines[lines.length - 1].trim();
  const statusCode = parseInt(lines[lines.length - 2].trim(), 10);
  const body = lines.slice(0, -2).join("\n");
  log("CURL", `curl <- ${statusCode} (${body.length} bytes)`);
  if (statusCode >= 400) {
    const retryMsg = retryAfterStr ? ` — retry after ${retryAfterStr}s` : "";
    throw new Error(`HTTP ${statusCode}${retryMsg}`);
  }
  return body;
}

const CFFI_SCRIPT = fileURLToPath(new URL("../scripts/cffi_fetch.py", import.meta.url));
let _pythonBin;
...
async function getPythonBin() {
  ...
  for (const bin of ["python3", "python"]) {
    try {
      await execFileAsync(bin, ["-c", "import curl_cffi"], { timeout: 5000 });
      _pythonBin = bin;
      log("CFFI", `curl_cffi available via ${bin}`);
      return bin;
    } catch {}
  }
  _pythonBin = null;
...
/**
 * Fetches a URL using curl_cffi (Python) with Chrome TLS/HTTP2 fingerprint
 * impersonation. Requires Python 3.10+ and `pip install curl_cffi`.
 *
 * @param {string} url - Target URL.
 * @param {number} timeout - Request timeout in milliseconds.
 * @returns {Promise<string>} Response body.
 * @throws {Error} If curl_cffi is not available or the request fails.
 */
async function fetchWithCurlCffi(url, timeout) {
  const pythonBin = await getPythonBin();
  if (!pythonBin) throw new Error("curl_cffi not available");

  log("CFFI", `curl_cffi -> ${url} (timeout: ${timeout}ms)`);
  const { stdout } = await execFileAsync(pythonBin, [CFFI_SCRIPT, url, String(timeout), "pt-BR,pt;q=0.9,en-US;q=0.8,en;q=0.7"], { maxBuffer: 10 * 1024 * 1024, timeout: timeout + 10000 });
  const lines = stdout.split("\n");
  const retryAfterStr = lines[lines.length - 1].trim();
  const statusCode = parseInt(lines[lines.length - 2].trim(), 10);
  const body = lines.slice(0, -2).join("\n");
  log("CFFI", `curl_cffi <- ${statusCode} (${body.length} bytes)`);
  if (statusCode >= 400) {
    const retryMsg = retryAfterStr ? ` — retry after ${retryAfterStr}s` : "";
    throw new Error(`HTTP ${statusCode}${retryMsg}`);
  }
...
/**
 * Fetches a page body using a tiered strategy:
 * 1. Native fetch (fastest, but easily fingerprinted)
 * 2. curl_cffi with Chrome impersonation (if Python + curl_cffi installed)
 * 3. Plain curl as last resort
 *
 * @param {string} url - Target URL.
 * @param {number} timeout - Request timeout in milliseconds.
 * @returns {Promise<string>} Response body.
...
async function fetchPage(url, timeout) {
  try {
    ...
    try {
      return await fetchWithCurlCffi(url, timeout);
    } catch (cffiErr) {
      log("CFFI", `curl_cffi failed or unavailable: ${cffiErr.message}`);
...
/**
 * Tests whether an item's text corpus contains every token in the list.
 *
