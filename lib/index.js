...
export async function search(query, options = {}) {
  const { limit = DEFAULT_LIMIT, timeout = DEFAULT_TIMEOUT, sort, concurrency: rawConcurrency = DEFAULT_CONCURRENCY, city, category, minPrice, maxPrice, radius, strict = false, noRateLimit = false } = options;
  const concurrency = noRateLimit ? rawConcurrency : Math.min(rawConcurrency, RATE_LIMIT_CONCURRENCY);
  log("SEARCH", `search("${query}") called`, { limit, sort, city, category, minPrice, maxPrice, radius, strict, noRateLimit, concurrency });
